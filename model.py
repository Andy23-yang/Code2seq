...
class Model:
    ...
    def continue_train(self):
        ...
        # print('Number of trainable params:',
        #       np.sum([np.prod(v.get_shape().as_list()) for v in tf.trainable_variables()]))
        # tf.train.Saver.restore(self.sess, "model/csn/model_iter40")
        self.initialize_session_variables(self.sess)
        # print('Initalized variables')

        # saver = tf.train.import_meta_graph("model/csn/model_iter40.meta")
        # saver.restore(self.sess, "model/csn/model_iter40")
    ...
    def evaluate(self, release=False):
        ...
        ref_file_name = model_dirname + '/ref_test.txt'
        predicted_file_name = model_dirname + '/pred_test.txt'
        if not os.path.exists(model_dirname):
            os.makedirs(model_dirname)

        with open(model_dirname + '/log_test.txt', 'w') as output_file, open(ref_file_name, 'w') as ref_file, open(
                predicted_file_name,
                'w') as pred_file:
